...
void onTankJoin(char *msg){
	...
	memset(user[id],0,sizeof(USER_DATA));
...
void SceneGame(){
	...
	Sprite *tank[2];
	tank[0] = new Sprite("resource\\t1.png",1,1);
	tank[1] = new Sprite("resource\\t2.png",1,1);
	...
	while(!quit){
		...
		}
		{
			...
			for(itor=user.begin();itor!=user.end();++itor){
				tank[itor->second->direction]->Draw(
						itor->second->x,
						itor->second->y,
						200,100);
			}
			tank[player.direction]->Draw(player.x,player.y,200,100);
